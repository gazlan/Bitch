...
// from newer versions of "windows.h"
...
static void DumpSystemInformation(HANDLE LogFile)
{
   ...
   TCHAR    szWinVer[MAX_PATH]; 
   TCHAR    szMajorMinorBuild[MAX_PATH];
...
static const TCHAR* GetExceptionDescription(DWORD ExceptionCode)
{
   ...
   #define STATUS_ILLEGAL_VLM_REFERENCE     ((DWORD)0xC00002C0)     
   ////////////////////////////////////////////////////////////
   #endif
...
static void DumpStack(HANDLE LogFile,DWORD* pStack)
{
   ...
   __try
   {
      ...
      while ((pStack + 1) <= pStackTop)
      {
         ...
         if (!(++Count % StackColumns) || ((pStack + 2) > pStackTop))
         {
            hprintf(LogFile,_T("%08X "),*pStack);
            ++nDwordsPrinted;
...
static void DumpRegisters(HANDLE LogFile,PCONTEXT Context)
{
   ...
   hprintf(LogFile,_T("EDI:    %08X  ESI: %08X  EAX:   %08X\r\n"),Context->Edi,Context->Esi,Context->Eax);
   hprintf(LogFile,_T("EBX:    %08X  ECX: %08X  EDX:   %08X\r\n"),Context->Ebx,Context->Ecx,Context->Edx);
   hprintf(LogFile,_T("EIP:    %08X  EBP: %08X  SegCs: %08X\r\n"),Context->Eip,Context->Ebp,Context->SegCs);
   hprintf(LogFile,_T("EFlags: %08X  ESP: %08X  SegSs: %08X\r\n"),Context->EFlags,Context->Esp,Context->SegSs);
}
...
int __cdecl RecordExceptionInfo(PEXCEPTION_POINTERS pExceptPtrs,LPCTSTR lpszMessage)
{
   ...
   HANDLE   hLogFile = CreateFile(szModuleName,GENERIC_WRITE,0,0,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL | FILE_FLAG_WRITE_THROUGH,0);
   ...
   TCHAR    szCrashModulePathName[MAX_PATH * 2 + 1];
   ...
   // compatible format.
   hprintf(hLogFile,_T("%s caused %s (%08X) \r\nin module %s at %04X:%08X.\r\n\r\n"),szFileName,GetExceptionDescription(Exception->ExceptionCode),Exception->ExceptionCode,pszCrashModuleFileName,Context->SegCs,Context->Eip);
   ...
   // Create the file
   HANDLE      hMiniDumpFile = CreateFile(szModuleName,GENERIC_WRITE,0,NULL,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL | FILE_FLAG_WRITE_THROUGH,NULL);
   ...
   ::MessageBox(GetActiveWindow(),"See WTF in the 00_crash_report.txt","Shit happens!",MB_OK | MB_ICONSTOP);
